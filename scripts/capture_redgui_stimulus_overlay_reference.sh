...
output_dir="${3:-$repo_root/docs/reference/phase5n/linux}"
width="${4:-1920}"
height="${5:-1080}"
layout="${CRIMSON_PHASE5N_REFERENCE_IMGUI_INI:-$repo_root/docs/reference/phase5l/linux/arranged_reference_1920x1080.imgui.ini}"
...
CRIMSON_REFERENCE_UI_STATE=stimulus-overlay \
CRIMSON_REFERENCE_FRAME=1024 \
CRIMSON_REFERENCE_UI_TIMEOUT="${CRIMSON_PHASE5N_REFERENCE_TIMEOUT:-180}" \
CRIMSON_REFERENCE_IMGUI_INI="$layout" \
"$repo_root/scripts/capture_redgui_workspace_reference.sh" \
    "$redgui" "$zarr" "$output_dir" \
    stimulus_overlay_exact_f001024_1920x1080 "$width" "$height"

echo "Linux stimulus-overlay reference captured in $output_dir"
