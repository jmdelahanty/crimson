...
output_dir="${1:-${CRIMSON_MACOS_REFERENCE_OUTPUT_DIR:-$repo_root/docs/reference/phase5n/macos}}"
timeout="${CRIMSON_MACOS_REFERENCE_TIMEOUT:-180}"
logical_size="${CRIMSON_MACOS_REFERENCE_SIZE:-1920x1080}"
frame=1024
stem=stimulus_overlay_exact_f001024_1920x1080
...
if ! jq -e \
    ...
    '($size | split("x") | map(tonumber)) as $dimensions |
     ...
     .stimulus_camera_overlay.step_index == null and
     .stimulus_camera_overlay.grating_direction_camera_deg == null and
     .stimulus_camera_overlay.primitive_count == 1 and
     .stimulus_camera_overlay.text_count == 1 and
     (.stimulus_camera_overlay.primitives | length) == 1 and
     (.stimulus_camera_overlay.text | length) == 1 and
     .stimulus_camera_overlay.primitives[0].layer == "event_panel" and
     .stimulus_camera_overlay.text[0].layer == "event_text" and
     .stimulus_camera_overlay.text[0].content == $expected_text and
     .stimulus_camera_overlay.descriptor.run_name ==
       "stimulus_external_ipc_20260616_01" and
     .stimulus_camera_overlay.descriptor.frame_count == 140035 and
     .stimulus_camera_overlay.descriptor.event_count == 984 and
     .stimulus_camera_overlay.descriptor.step_count == 1 and
     (.stimulus_camera_overlay.semantic_signature |
      startswith("crimson.stimulus-camera-overlay-semantics.v1"))' \
    "$marker" >/dev/null; then
    echo "Mac stimulus-overlay reference marker validation failed: $marker" >&2
    jq '{platform, state, target_frame, presented_frame, stable_frames,
         stimulus_camera_overlay}' "$marker" >&2 || true
    tail -n 160 "$log" >&2 || true
    exit 4
fi
if [[ ! -f "$image" ]]; then
    echo "Mac stimulus-overlay reference image was not written: $image" >&2
    exit 4
fi
...
jq -n \
    --arg schema "crimson.phase5n.stimulus-overlay-capture-set.v1" \
    ...
    '{schema: $schema, captured_at: $captured_at,
      ...
      capture: {state: "stimulus-overlay", frame: $frame,
                stem: "stimulus_overlay_exact_f001024_1920x1080"}}' \
    >"$manifest"

echo "Mac stimulus-overlay reference captured in $output_dir"
