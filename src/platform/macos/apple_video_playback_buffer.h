...
struct AppleVideoPlaybackBufferMetrics {
  ...
  uint64_t clip_switches = 0;
  int64_t last_decoded_frame = -1;
  int64_t active_clip_index = -1;
...
public:
  ...
  AppleVideoPlaybackBuffer &
  operator=(const AppleVideoPlaybackBuffer &) = delete;

  bool open(const std::string &path, const std::string &stream_id,
            size_t capacity, std::string *error = nullptr);
  bool openClipIndex(const std::string &index_path,
                     const std::string &stream_id, size_t capacity,
  ...
  std::optional<AppleDecodedVideoFrame> frameForTarget(int64_t frame_number,
