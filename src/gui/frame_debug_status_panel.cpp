...
void drawEyeMaskTab(
    ...
    if (context.zarr_loader.hasEyeMasks()) {
        ...
        if (context.detection_details != nullptr &&
            ...
            if (masks_with_axes > 0) {
                ImGui::Text("Current frame masks with Feret axes: %zu",
                            masks_with_axes);
            }
        } else {
            ImGui::TextDisabled("Current frame eye-mask details unavailable");
        }
...
void drawFrameDebugStatusPanel(const FrameDebugWindowContext& context,
    ...
    if (context.zarr_loader.hasEyeMasks() || eye_mask_artifact != nullptr) {
        if (ImGui::BeginTabItem("Eye Masks")) {
            state.active_tab = FrameInspectTab::EyeMasks;
            drawEyeMaskTab(context, eye_mask_artifact);
            ImGui::EndTabItem();
        }
    }
