...
bool ZarrDetectionLoader::loadZarrFile(const std::string& filepath,
    ...
    try {
        ...
        const auto clipped_resolver_start = std::chrono::steady_clock::now();
        if (clipped_resolver_.load(root_path_, std::string(), clipped_error)) {
            if (clipped_startup_trace) {
                std::cout << "[ClippedStartup] resolver_load_ms="
                          << elapsedMs(clipped_resolver_start) << std::endl;
            }
            data_.has_clipped_collection = true;
            if (clipped_resolver_.totalParentFrames() > data_.total_frames) {
                data_.total_frames = clipped_resolver_.totalParentFrames();
            }
            std::cout << "  Loaded clipped collection '"
                      << clipped_resolver_.collectionId() << "' ("
                      << clipped_resolver_.selectedRunCount()
                      << " selected runs, "
                      << clipped_resolver_.mappedFrameCount()
                      << " mapped frames)" << std::endl;
            const auto clipped_primary_start =
                std::chrono::steady_clock::now();
            if (!loadClippedRefinedCollectionAsPrimary(store)) {
                std::cout << "  Clipped refined instances were not loaded; "
                          << "opening clipped collection metadata only"
                          << std::endl;
            }
            if (clipped_startup_trace) {
                std::cout << "[ClippedStartup] primary_load_ms="
                          << elapsedMs(clipped_primary_start) << std::endl;
            }
        } else if (!clipped_error.empty()) {
            if (clipped_startup_trace) {
                std::cout << "[ClippedStartup] resolver_unavailable_ms="
                          << elapsedMs(clipped_resolver_start) << std::endl;
            }
    ...
    try {
        ...
        if (open_result.ok()) {
            ...
            for (size_t row = 0; row < rows; ++row) {
                out[row][0] = array(static_cast<ts::Index>(row), 0);
                out[row][1] = array(static_cast<ts::Index>(row), 1);
                out[row][2] = array(static_cast<ts::Index>(row), 2);
                out[row][3] = array(static_cast<ts::Index>(row), 3);
            }
            return true;
        }
        ...
        out.resize(rows);
        for (size_t row = 0; row < rows; ++row) {
            out[row][0] = static_cast<float>(
                array(static_cast<ts::Index>(row), 0));
            out[row][1] = static_cast<float>(
                array(static_cast<ts::Index>(row), 1));
            out[row][2] = static_cast<float>(
                array(static_cast<ts::Index>(row), 2));
            out[row][3] = static_cast<float>(
                array(static_cast<ts::Index>(row), 3));
        }
        return true;
...
bool ZarrDetectionLoader::loadClippedRefinedCollectionAsPrimary(
    const ts::kvstore::KvStore& store) {
    if (!clipped_resolver_.loaded() ||
        clipped_resolver_.selectedRunCount() == 0) {
        return false;
    }

    const size_t total_parent_frames = clipped_resolver_.totalParentFrames();
    if (total_parent_frames == 0) {
        return false;
    }

    InterpolationRunData stage;
    stage.run_name = clipped_resolver_.collectionId();
    ...
    size_t scaled_clipped_bbox_runs = 0;
    ...
    const auto& selected_runs = clipped_resolver_.selectedRuns();
    for (const auto& selected : selected_runs) {
        ...
        if (!readInt32Array(store, instances_base + "frame_indices",
                            clip_frame_indices)) {
            read_frame_indices_ms += elapsedMs(phase_start);
        ...
        if (!selected.detect_group_path.empty()) {
            ...
            read_attrs_ms += elapsedMs(phase_start);
        }
        phase_start = std::chrono::steady_clock::now();
        ...
        if (geometry.source_width > 0 && geometry.source_height > 0) {
            ...
            const bool image_dims_match_inference =
                data_.image_width == geometry.inference_width &&
                data_.image_height == geometry.inference_height;
            if (no_image_dims || image_dims_match_inference) {
                data_.image_width = geometry.source_width;
                data_.image_height = geometry.source_height;
            }
        }
        ...
        phase_start = std::chrono::steady_clock::now();
        if (readFloatMatrix(store, instances_base + "bbox_img_xyxy", run_boxes)) {
            read_bbox_img_ms += elapsedMs(phase_start);
            run_boxes_are_pixel_xyxy = true;
            const int bbox_target_width =
                geometry.source_width > 0 ? geometry.source_width : data_.image_width;
            const int bbox_target_height =
                geometry.source_height > 0 ? geometry.source_height : data_.image_height;
            ...
            if (!metadata_declares_source_pixels) {
                ...
                phase_start = std::chrono::steady_clock::now();
                readFloatMatrix(store, instances_base + "bbox_norm_coords",
                                run_norm_boxes);
                read_bbox_norm_ms += elapsedMs(phase_start);
                if (shouldScaleInferencePixelBoxesToSource(
                    ...
                    scalePixelXyxyBoxes(run_boxes, geometry.inference_width,
                                        ...
                                        geometry.source_height);
                    ++scaled_clipped_bbox_runs;
                    if (first_scaled_geometry.empty()) {
                        first_scaled_geometry =
            ...
            read_bbox_img_ms += elapsedMs(phase_start);
            phase_start = std::chrono::steady_clock::now();
            if (readFloatMatrix(store, instances_base + "bbox_norm_coords",
                                run_boxes)) {
        ...
        const bool run_has_scores =
            (readFloatArray(store, instances_base + "confidence_scores", run_scores) ||
             readFloatArray(store, instances_base + "scores", run_scores)) &&
            run_scores.size() == run_boxes.size();
        read_scores_ms += elapsedMs(phase_start);
        any_scores = any_scores || run_has_scores;

        std::vector<int32_t> run_class_ids;
        phase_start = std::chrono::steady_clock::now();
        const bool run_has_class_ids =
            readInt32Array(store, instances_base + "class_ids", run_class_ids) &&
            run_class_ids.size() == run_boxes.size();
        read_class_ids_ms += elapsedMs(phase_start);
        ...
        if (!all_rows_raw_detect) {
            ...
            run_has_source_kind_codes =
                readInt32Array(store, instances_base + "source_kind_codes",
                               run_source_kind_codes) &&
                run_source_kind_codes.size() == run_boxes.size();
            read_source_kind_ms += elapsedMs(phase_start);
        }
        ...
        append_rows_ms += elapsedMs(phase_start);
        ++loaded_runs;
        total_clip_rows += run_boxes.size();
    }
    ...
    if (scaled_clipped_bbox_runs > 0) {
        std::cout << "  Scaled clipped bbox_img_xyxy geometry from inference "
                  << "pixels to source video pixels for "
                  << scaled_clipped_bbox_runs << " run"
                  << (scaled_clipped_bbox_runs == 1 ? "" : "s");
        if (!first_scaled_geometry.empty()) {
            std::cout << " (" << first_scaled_geometry << ")";
        }
        std::cout << std::endl;
    }
    ...
    const double sort_reorder_ms = elapsedMs(sort_start);
    ...
    const double offsets_ms = elapsedMs(offsets_start);
    ...
    cacheDetectionStage(std::move(stage), DetectionDataset::RefinedRoot);
    if (!applyDetectionDataset(data_.refined_root_dataset,
                               DetectionDataset::RefinedRoot)) {
        return false;
    }
    const double cache_apply_ms = elapsedMs(apply_start);
    std::cout << "  Using clipped refined collection '"
              << clipped_resolver_.collectionId()
              << "' as primary detections ("
              << data_.bbox_norm_coords.size() << " boxes over "
              << data_.total_frames << " parent frames)" << std::endl;
    if (clipped_startup_trace) {
        std::cout << "[ClippedStartup] primary_detail total_ms="
                  << elapsedMs(load_start)
                  << " loaded_runs=" << loaded_runs
                  << " selected_runs=" << selected_runs.size()
                  << " total_clip_rows=" << total_clip_rows
                  << " kept_boxes=" << data_.bbox_norm_coords.size()
                  << " read_frame_indices_ms=" << read_frame_indices_ms
                  << " read_attrs_ms=" << read_attrs_ms
                  << " read_bbox_img_ms=" << read_bbox_img_ms
                  << " read_bbox_norm_ms=" << read_bbox_norm_ms
                  << " read_scores_ms=" << read_scores_ms
                  << " read_class_ids_ms=" << read_class_ids_ms
                  << " read_source_kind_ms=" << read_source_kind_ms
                  << " append_rows_ms=" << append_rows_ms
                  << " sort_reorder_ms=" << sort_reorder_ms
                  << " offsets_ms=" << offsets_ms
                  << " cache_apply_ms=" << cache_apply_ms
                  << std::endl;
    }
    return true;
}
