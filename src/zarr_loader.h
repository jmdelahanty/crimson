...
struct ZarrDetectionData {
    ...
    // Using void* to avoid template issues in header
...
public:
    ...
    struct FrameDetections {
        std::vector<std::array<float, 4>> boxes;  // [x_min, y_min, x_max, y_max] or [x, y, w, h]
        std::vector<float> scores;
        std::vector<int32_t> class_ids;
        size_t frame_id;
    };
    FrameDetections getRawDetections(size_t frame_id) const;
...
private:
    ...
    bool loadDetectRunsFormat(const ts::kvstore::KvStore& store);
    ...
    std::array<float, 4> normalizeCoordinates(
        const std::array<float, 4>& box,
        int image_width,
        int image_height
    ) const;
...
// Standalone helper function for integration with existing code
