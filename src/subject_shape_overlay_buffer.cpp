...
struct SubjectShapeOverlayBuffer::Impl {
  ...
  std::unordered_map<
      int64_t, std::shared_ptr<const crimson::zarr::SubjectShapeOverlayResolution>>
      cache;
  std::deque<int64_t> cache_order;
  SubjectShapeOverlayBufferMetrics metrics;
  ...
  void publishLocked(
      int64_t frame,
      std::shared_ptr<const crimson::zarr::SubjectShapeOverlayResolution> value) {
...
SubjectShapeOverlayBuffer::SubjectShapeOverlayBuffer(
    std::shared_ptr<crimson::data::DataAccessScheduler> scheduler,
    std::string archive_identity)
...
SubjectShapeOverlayBuffer::~SubjectShapeOverlayBuffer() { close(); }

bool SubjectShapeOverlayBuffer::open(
    std::unique_ptr<crimson::zarr::SubjectShapeOverlayRepository> repository,
    size_t lookahead_frames, size_t cache_capacity, std::string* error) {
  close();
  if (!repository) {
    assignError(error, "SubjectShape repository is null");
    return false;
  }
  if (cache_capacity == 0) {
    assignError(error, "SubjectShape cache capacity must be positive");
    return false;
  }
  if (!impl_->scheduler || !impl_->scheduler->running()) {
    assignError(error, "SubjectShape data scheduler is unavailable");
    return false;
  }
...
void SubjectShapeOverlayBuffer::close() {
...
bool SubjectShapeOverlayBuffer::isOpen() const {
  std::lock_guard<std::mutex> lock(impl_->mutex);
  return impl_->repository && !impl_->stopping && impl_->scheduler &&
         impl_->scheduler->running();
}

bool SubjectShapeOverlayBuffer::requestFrame(int64_t camera_frame,
  ...
  if (!impl_->repository || impl_->stopping) {
    assignError(error, "SubjectShape buffer is not open");
    return false;
  }
  if (camera_frame < 0 || full_frame_width <= 0 || full_frame_height <= 0) {
    assignError(error, "SubjectShape frame request is invalid");
    return false;
  }
  ...
  for (int64_t frame = camera_frame;; frame += direction) {
    if (impl_->cache.find(frame) == impl_->cache.end()) {
      ...
      const auto outcome = impl_->scheduler->submit(
          ...
          [this, frame, full_frame_width, full_frame_height, generation](
            ...
            auto shared = std::make_shared<
                const crimson::zarr::SubjectShapeOverlayResolution>(
                std::move(resolved));
            ...
            switch (shared->status) {
              case crimson::zarr::SubjectShapeOverlayStatus::Mapped:
                ++impl_->metrics.resolved_frames;
                status = crimson::data::DataResultStatus::Ready;
                break;
              case crimson::zarr::SubjectShapeOverlayStatus::Missing:
              case crimson::zarr::SubjectShapeOverlayStatus::OutOfRange:
                ++impl_->metrics.missing_frames;
                status = crimson::data::DataResultStatus::Missing;
                break;
              case crimson::zarr::SubjectShapeOverlayStatus::InvalidDimensions:
              case crimson::zarr::SubjectShapeOverlayStatus::ReadFailed:
      ...
      if (frame == camera_frame && !outcome.accepted()) {
        assignError(error, "SubjectShape scheduler rejected current frame");
        return false;
      }
    }
...
bool SubjectShapeOverlayBuffer::waitForFrame(
...
SubjectShapeOverlayBuffer::frame(int64_t camera_frame) const {
  std::lock_guard<std::mutex> lock(impl_->mutex);
  const auto found = impl_->cache.find(camera_frame);
  return found == impl_->cache.end() ? nullptr : found->second;
}

crimson::zarr::SubjectShapeOverlayDescriptor SubjectShapeOverlayBuffer::descriptor()
    const {
  std::lock_guard<std::mutex> lock(impl_->mutex);
  return impl_->descriptor;
}

SubjectShapeOverlayBufferMetrics SubjectShapeOverlayBuffer::metrics() const {
  std::lock_guard<std::mutex> lock(impl_->mutex);
