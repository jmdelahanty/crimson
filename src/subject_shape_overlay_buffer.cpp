  ...
  void run() {
    while (true) {
      ...
      const double elapsed_ms = std::chrono::duration<double, std::milli>(
                                    ...
                                    .count();
      auto shared =
          std::make_shared<const crimson::zarr::SubjectShapeOverlayResolution>(
              std::move(resolution));
      {
...
bool SubjectShapeOverlayBuffer::open(
  ...
  close();
  if (!repository) {
    assignError(error, "Subject-shape repository is null");
    return false;
  }
  if (cache_capacity == 0) {
    assignError(error, "Subject-shape cache capacity must be positive");
    return false;
  }
  {
...
bool SubjectShapeOverlayBuffer::requestFrame(int64_t camera_frame,
  ...
  if (!impl_->repository || impl_->stopping) {
    assignError(error, "Subject-shape buffer is not open");
    return false;
  }
  if (camera_frame < 0 || full_frame_width <= 0 || full_frame_height <= 0) {
    assignError(error, "Subject-shape frame request is invalid");
    return false;
  }
...
crimson::zarr::SubjectShapeOverlayDescriptor
SubjectShapeOverlayBuffer::descriptor() const {
  std::lock_guard<std::mutex> lock(impl_->mutex);
