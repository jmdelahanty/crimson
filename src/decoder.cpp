...
void software_decoder_process(DecoderContext *dc_context,
    ...
    if (!capture.isOpened()) {
        std::cerr << "[Decoder] Failed to open software decoder for "
                  << video_path << std::endl;
        return;
    }

    const int reported_width =
        static_cast<int>(capture.get(cv::CAP_PROP_FRAME_WIDTH));
    const int reported_height =
    ...
    if (width <= 0 || height <= 0) {
        std::cerr << "[Decoder] Software decoder reported invalid dimensions for "
                  << video_path << std::endl;
        return;
    }
    ...
    while (!(dc_context->stop_flag)) {
        ...
        if (has_seek_request) {
            ...
            const bool seek_ok =
                capture.set(cv::CAP_PROP_POS_FRAMES,
                            static_cast<double>(requested_frame));
            if (!seek_ok) {
                std::lock_guard<std::mutex> lock(g_seek_info_mutex);
                seek_info->seek_frame = requested_frame;
                seek_info->settled_seek_id = active_seek_id;
                seek_info->seek_done = true;
                pending_seek_done = false;
                continue;
        ...
        const double decode_wait_ms = decoder_duration_ms(
            std::chrono::steady_clock::now() - wait_start);
        ...
        cv::Mat frame_bgr;
        const auto decode_submit_start = std::chrono::steady_clock::now();
        const bool read_ok = capture.read(frame_bgr);
        const double decode_submit_ms = decoder_duration_ms(
            std::chrono::steady_clock::now() - decode_submit_start);
        if (!read_ok || frame_bgr.empty()) {
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
            continue;
        }

        const auto decode_pipeline_start = std::chrono::steady_clock::now();
        double decode_convert_ms = 0.0;
        double decode_write_ms = 0.0;
        ...
        case 1:
            ...
            break;
        default:
            std::cerr << "[Decoder] Unsupported channel count in software decode: "
                      << frame_bgr.channels() << std::endl;
            continue;
        }
        if (frame_rgba.cols != width || frame_rgba.rows != height) {
            cv::resize(frame_rgba, frame_rgba, cv::Size(width, height), 0.0,
                       0.0, cv::INTER_LINEAR);
        }
        decode_convert_ms = decoder_duration_ms(
            std::chrono::steady_clock::now() - convert_start);
        ...
        const auto write_start = std::chrono::steady_clock::now();
        if (use_cpu_buffer) {
            std::memcpy(display_buffer[buffer_head].frame, frame_rgba.data,
                        frame_bytes);
        } else {
        ...
        display_buffer[buffer_head].pitch_bytes = width * 4;
        display_buffer[buffer_head].frame_bytes = frame_bytes;
        display_buffer[buffer_head].color_matrix = ColorSpaceStandard_BT709;
        display_buffer[buffer_head].format = PictureBufferFormat::RGBA32;
        latest_decoded_frame[cam_name].store(frame_number);
        dc_context->decoding_flag = true;

        decoder_perf->demux_ms.store(0.0);
        decoder_perf->decode_submit_ms.store(decode_submit_ms);
        decoder_perf->nv12_to_rgba_ms.store(decode_convert_ms);
        decoder_perf->buffer_wait_ms.store(decode_wait_ms);
        decoder_perf->frame_write_ms.store(decode_write_ms);
        decoder_perf->frame_total_ms.store(decoder_duration_ms(
            std::chrono::steady_clock::now() - decode_pipeline_start));
        decoder_perf->published_frame.store(frame_number);

        if (pending_seek_done) {
            std::lock_guard<std::mutex> lock(g_seek_info_mutex);
            seek_info->seek_frame = static_cast<uint64_t>(frame_number);
            seek_info->settled_seek_id = pending_seek_id;
            seek_info->seek_done = true;
            pending_seek_done = false;
        }
