...
- Official storage format is **Zarr v3**.
- Metadata source of truth is `zarr.json` (including the `attributes` object).
- Reader compatibility fallback: Crimson currently also checks `.zattrs`
  when `zarr.json` attributes are unavailable.
- Producer guidance: write v3 metadata to `zarr.json`; do not depend on
  `.zattrs` for new outputs.
...
Current Crimson status:

- Raw eye-mask runs are part of the Palette schema.
- Crimson's current review/display target is still
  `refined_eye_masks_runs/<latest>/masks_roi`.
- Raw probability-mask support should be treated as future-capable, not a
  required current UI surface.
...
| `detection_indices` *(optional)* | `(n_rois,)` | `int32` | Index into `crop_runs/<run>/roi_images` |
| `masks_roi` *(optional)* | `(n_rois, C, H, W)` | `uint8` | Binary masks; raw runs may be union (`C=1`) or left/right (`C=2`) |
| `mask_probs_roi` *(optional)* | `(n_rois, C, H, W)` | `float16/float32/uint8` | Semantic probabilities in `[0,1]`; `uint8` uses linear decode `stored / 255` |
...
Attributes: `source_crop_run`, `source_keypoint_run`, `source_keypoint_group` (defaults to `refined_keypoints_runs` when present), `method`, model info,
thresholds, separation limits, `successful_eyes`, `successful_roi_pairs`, `reason_counts`,
`ellipse_angle_units`, `ellipse_fit_method`, `eye_labels`,
`segmenter_label_mode`, `probabilities_dtype`, `probabilities_encoding`,
`recommended_probability_threshold`, `recommended_probability_threshold_review`,
`duration_seconds`.
...
Current Crimson target:

- This is the current required eye-mask review/display source.
- Crimson should prefer `refined_eye_masks_runs/<latest>/masks_roi`.
- Refined probabilities are optional and should not be required for a healthy
  refined run.
...
| `detection_indices` *(optional)* | `(n_rois,)` | `int32` | Copied from source when present |
| `mask_probs_roi_refined` *(optional)* | `(n_rois, 2, H, W)` | `float16/float32/uint8` | Refined probabilities when explicitly written; semantic values remain `[0,1]` |
| `contour_left_ptr` | `(n_rois,)` | `int32` | Pointer into `contours_left` |
| `contour_left_len` | `(n_rois,)` | `int32` | Number of points for left eye contour |
| `contour_right_ptr` | `(n_rois,)` | `int32` | Pointer into `contours_right` |
| `contour_right_len` | `(n_rois,)` | `int32` | Number of points for right eye contour |
...
`source_keypoint_group`, `source_keypoint_run`, `source_crop_run`).
...
Threshold provenance attrs may include:
...
## Crimson Compatibility Notes

- Crimson supports analysis archives that are created before inference has
  produced `detect_runs/`.
- When `detect_runs/` is absent, Crimson opens the archive in
  metadata/stimulus-only mode:
  - detection overlays are unavailable
  - stimulus/alignment and metadata inspection remain available when present
- During directory discovery, Crimson prioritizes archives in this order:
  1. `zarr_purpose=analysis` with `detect_runs/`
  2. any archive with `detect_runs/`
  3. `zarr_purpose=analysis` without `detect_runs/` (metadata-only fallback)
- If multiple archives tie at the highest priority, Crimson requires an
  explicit archive path override instead of silently picking one.
- Missing optional groups (for example `analysis/stimulus_runs/`) do not block
  opening an otherwise valid archive.
