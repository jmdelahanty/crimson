...
Status: fixture and acceptance contract frozen; 2,048-frame integration pair
published and accepted; full-duration Stage 1 matrix not yet run
...
Palette published the paired 2,048-frame integration archives on 2026-07-26.
Crimson's fresh-process integration runner accepted both layouts through the
shared asynchronous loader, first presentations, complete fixture traversal,
overlay adaptation, and seek cancellation. Real Metal playback smokes also
accepted both layouts through frame 300. This publication remains
integration-only: the deterministic five-process full-duration runner,
physical I/O and full-duration RSS capture, and reducer remain open.
...
## 2,048-Frame Integration Checkpoint

Palette fixture:

```text
/Volumes/johnsonlab/jeremy/recordings/.palette_benchmarks/
canonical_detection_storage/full_analysis/
sleepyfish_cam2010095_integration_2048_v1/{regular,hybrid}.zarr
```

Crimson runs `canonical_detection_full_archive_integration_gate` with the
explicit detection run and video path. The fixture lives outside its source
recording, so its persisted `/groups/...` video path cannot resolve through the
benchmark root on macOS. This is expected for this fixture tier; the runner and
the application command use the frozen explicit mounted video path.

The retained 2026-07-26 pair passed these integration assertions:

- all required repositories reached Ready through the asynchronous loader;
- frame-zero keypoints, subject masks, subject shape, eye geometry, crop
  geometry, motion, eye-angle, and tail presentations resolved;
- all 2,048 canonical frames produced 2,048 detections and 2,048 shared-scene
  boxes with identical regular/hybrid digest `e7665c85d8913ff5`;
- the canonical adapter used one root metadata read, nine consolidated
  declarations, four exact typed opens, zero fallbacks, and one retained
  offsets read;
- the shared scheduler reached four active workers, detection fields overlapped
  in flight, the seek burst cancelled 14 stale requests in each run, and no
  stale result or failed work published; and
- the active TensorStore cache was the unchanged 64 MiB production pool.

The regular and hybrid Metal smokes each presented frame 300 with zero skipped
source frames, zero late presentations, zero maximum frame lag, and all
required overlay/timeline products active. Peak process RSS was 771.6 MiB for
regular and 778.8 MiB for hybrid.

Evidence is retained in:

```text
docs/diagnostics/crimson_macos_phase5o4_full_archive_integration_2026-07-26.md
docs/diagnostics/crimson_macos_phase5o4_full_archive_integration_regular_2026-07-26.json
docs/diagnostics/crimson_macos_phase5o4_full_archive_integration_hybrid_2026-07-26.json
```

Because every canonical array in the small fixture fits in a single inner
chunk in both candidates, these timings do not compare the production regular
and hybrid physical layouts. This checkpoint proves schema, codec, loader,
presentation, and cancellation compatibility only.
