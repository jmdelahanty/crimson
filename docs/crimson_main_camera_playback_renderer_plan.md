...
The first playback-renderer prototype has now landed and was profiled on the
Windows RTX A1000 laptop.

Representative A/B result with otherwise comparable settings:

- main camera: `GPU Buffer`, size `8`
- main storage format: `NV12`
- stimulus: software decode + `GPU` buffer
- playback preview scale: `1x`

Compared runs:

- `standard` playback renderer
- `lightweight` playback renderer

Observed improvement:

- mean playback speed: `0.862x` -> `0.943x`
- median playback speed: `0.867x` -> `0.967x`
- p90 playback speed: `0.874x` -> `1.002x`
- samples at `>= 0.95x`: `4.3%` -> `59.7%`
- mean `gl_draw_ms`: `16.77 ms` -> `16.16 ms`
- mean `frame_loop_ms`: `17.30 ms` -> `16.85 ms`
- mean ImGui draw-command count: `18.8` -> `13.4`

Important detail:

- the lightweight-renderer run used a slightly larger camera viewport

So the improvement is real, and if anything a bit stronger than the raw numbers
first suggest.

### What Phase 2 Means

The prototype established three important things:

1. a cheaper playback-specific renderer is the right optimization direction
2. the laptop is now close enough to real time that another moderate render win
   may plausibly reach or stabilize `1.0x`
3. the remaining bottleneck is still render cost, not upload, steady decode,
   or stimulus playback

But the prototype did **not** fully solve the laptop:

- `gl_draw_ms` still sits close to the `60 fps` frame budget
- mean playback is still below `1.0x`

So the next step should still be render-focused.

### Recommended Next Path

#### Step 1: Add A True Playback Render Scale To The Lightweight Renderer

Add a new playback-only render scale for the lightweight renderer, for example:
...
- it directly targets the remaining render bottleneck
- it is a smaller and safer change than replacing the camera widget entirely
- the earlier preview-scale work did not help enough because it still rendered
  through a full-size presentation path
- the lightweight renderer already proved that a modest presentation-path
  reduction can move playback much closer to real time

This should be implemented as a **real playback render target scale**, not just
texture mip bias or source sampling changes.

#### Step 2: If Needed, Build A Playback-Only Camera Widget

If Step 1 is not enough:

- keep the current ImPlot path for paused/editing workflows
- build a playback-only camera widget that preserves zoom/pan semantics and
  overlay alignment

Why this is the next fallback:

- it should cut more of the remaining plot/widget presentation cost
- it avoids regressing paused inspection
- it keeps the user-visible zoom workflow intact during playback

#### Step 3: If Needed, Move To Direct NV12 Shader Presentation

Longer-term:

- keep the main camera in `NV12`
- present it directly through a shader-based playback path
- avoid presenting a temporary full-frame `RGBA` texture where possible

Why this is later rather than next:

- it is a larger architectural change
- the current evidence suggests there is still smaller render-path work worth
  attempting first

### What Not To Prioritize Next

The current evidence argues against spending the next cycle on:

- main-camera software decode
  - FFmpeg benchmarking showed it is not the better path for the
    `4512x4512 @ 60 fps` camera stream
- more upload optimization
  - upload is already negligible in the current `GPU Buffer` path
- VSync work
  - swap time is small; the frame budget is being missed before swap
- ROI-only rendering
  - the zoomed-playback trace showed that tight zoom did not materially reduce
    draw cost on its own because the viewport stayed the same size
