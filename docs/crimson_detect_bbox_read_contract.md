...
Reader behavior:
1. If `reason_bytes` exists, decode and use it for UI/status labeling.
2. Else if `reason` exists, use it for UI/status labeling.
3. Else if `detection_source` exists, derive:
   - `0 -> clean`
   - `1 -> interpolated`
