...
bool WriteAttributes(const std::filesystem::path& group,
...
bool TestAuthoritativeInventory(bool zarr_v2) {
  ...
  CHECK(WriteAttributes(
      fixture.archive_root / "analysis/acquisition_video_streams",
      Inventory(full), zarr_v2));
  CHECK(WriteAttributes(
      fixture.archive_root / "analysis/acquisition_video_streams/streams/full",
      full, zarr_v2));
...
bool TestForeignAbsoluteInventoryPath() {
  ...
  std::string error;
  auto descriptor = crimson::zarr::DiscoverAffiliatedVideo(
...
bool TestAuthoritativePrecedenceAndValidation() {
  {
    ...
    CHECK(WriteAttributes(fixture.archive_root, {{"source_path", "legacy.mp4"}}));
    CHECK(WriteAttributes(
        fixture.archive_root / "analysis/acquisition_video_streams",
        Inventory(full)));
    CHECK(WriteAttributes(
...
int main() {
  if (!TestAuthoritativeInventory(false) ||
      !TestAuthoritativeInventory(true) ||
      !TestForeignAbsoluteInventoryPath() ||
      !TestAuthoritativePrecedenceAndValidation() ||
      !TestLegacyPrecedence() ||
      !TestContractPathsDoNotUseLegacyFallbacks() ||
      !TestUnavailableInventoryFallsBack() ||
      !TestPathFailuresAndCleanAbsence() ||
      !TestUnavailableArchiveRoot()) {
    return 1;
  }
