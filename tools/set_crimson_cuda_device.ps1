...
function Get-CudaDeviceConfigPath {
    ...
    $fallbackPath = Join-Path $AppRoot "cuda_device.json"
    return $fallbackPath
}
...
function Describe-Device {
    ...
    return ("GPU {0} | {1} | {2} MiB | driver {3}" -f `
        $Device.index, $Device.name, $Device.memory_mb, $Device.driver_version)
}
...
if ($existingPreference -and
    ...
    $savedDevice = $devices | Where-Object { $_.index -eq $savedIndex } |
...
$selectedDevice = $null
if ($DeviceIndex -ge 0) {
    $selectedDevice = $devices | Where-Object { $_.index -eq $DeviceIndex } |
        Select-Object -First 1
