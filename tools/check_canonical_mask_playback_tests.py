...
def contour_events(contours=False):
    details = {
        "generation": 1, "query_frame": 1,
        "mask_enabled": not contours, "mask_frame": 1, "mask_state": "ready",
        "mask_outcome": "disabled" if contours else "drawn", "mask_error": "",
        "mask_expected_fills": 0 if contours else 4,
        "mask_actual_fills": 0 if contours else 4,
        "mask_decoded_cache_bytes": 8, "mask_decoded_cache_byte_budget": 1024,
        "contour_enabled": contours,
        "contour_frame": 1 if contours else -1,
        "contour_state": "ready" if contours else "pending",
        "contour_outcome": "drawn" if contours else "disabled",
        "contour_expected": 4 if contours else 0,
        "contour_actual": 4 if contours else 0,
        "contour_error": "", "contour_payload_reads": 1 if contours else 0,
    }
    return [
        {"event": "canonical_overlay_present", "details": details,
         "presenter": {"presented_frame": 1, "view_idx": 0},
         "playback": {"play_video": True}},
        {"event": "playback_smoke_pass",
         "details": {"start_frame": 1, "end_frame": 1,
                     "presented_frame": 1}},
    ]


class PlaybackCheckerTests(unittest.TestCase):
    def test_dense_only_preserves_mask_acceptance_and_zero_contour_work(self):
        sequence = contour_events()
        self.assertTrue(check(sequence, 1, 1, minimum_frames=1)["passed"])
        sequence[0]["details"]["contour_payload_reads"] = 1
        self.assertFalse(check(sequence, 1, 1, minimum_frames=1)["passed"])

    def test_contour_only_accepts_ready_first_draw_and_rejects_pending(self):
        sequence = contour_events(contours=True)
        self.assertTrue(check(sequence, 1, 1, minimum_frames=1,
                              require_contours=True)["passed"])
        sequence[0]["details"]["contour_state"] = "pending"
        self.assertFalse(check(sequence, 1, 1, minimum_frames=1,
                               require_contours=True)["passed"])

    def test_contour_only_rejects_fill_or_missing_payload(self):
        sequence = contour_events(contours=True)
        sequence[0]["details"]["mask_actual_fills"] = 1
        self.assertFalse(check(sequence, 1, 1, minimum_frames=1,
                               require_contours=True)["passed"])
        sequence[0]["details"]["mask_actual_fills"] = 0
        sequence[0]["details"]["contour_payload_reads"] = 0
        self.assertFalse(check(sequence, 1, 1, minimum_frames=1,
                               require_contours=True)["passed"])
