...
run_dependency_audit() {
    ...
    local audit_output
    audit_output="$(python3 - "$mode" "$write_dependency_manifest" "$app_root" "$release_json" "$ldd_file" <<'PY'
import collections
...
mode, manifest_path, app_root, release_json, ldd_file = sys.argv[1:6]
