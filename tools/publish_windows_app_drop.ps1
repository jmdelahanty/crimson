...
$stageRuntimeCheckScript = Require-Path -PathValue (Join-Path $StageRoot "check_crimson_runtime.ps1") -Label "Staged check_crimson_runtime.ps1"
$stageRuntimeCheckWrapper = Require-Path -PathValue (Join-Path $StageRoot "check_crimson_runtime.cmd") -Label "Staged check_crimson_runtime.cmd"
$stageCudaDeviceScript = Require-Path -PathValue (Join-Path $StageRoot "set_crimson_cuda_device.ps1") -Label "Staged set_crimson_cuda_device.ps1"
$stageCudaDeviceWrapper = Require-Path -PathValue (Join-Path $StageRoot "set_crimson_cuda_device.cmd") -Label "Staged set_crimson_cuda_device.cmd"
...
if (-not [string]::IsNullOrWhiteSpace($ReleaseName) -or $PublishCurrent -or $ArchiveExistingCurrent) {
    ...
    $publishedRuntimeCheckScript = Require-Path -PathValue (Join-Path $releaseRoot "check_crimson_runtime.ps1") -Label "Published release check_crimson_runtime.ps1"
    $publishedRuntimeCheckWrapper = Require-Path -PathValue (Join-Path $releaseRoot "check_crimson_runtime.cmd") -Label "Published release check_crimson_runtime.cmd"
    $publishedCudaDeviceScript = Require-Path -PathValue (Join-Path $releaseRoot "set_crimson_cuda_device.ps1") -Label "Published release set_crimson_cuda_device.ps1"
    $publishedCudaDeviceWrapper = Require-Path -PathValue (Join-Path $releaseRoot "set_crimson_cuda_device.cmd") -Label "Published release set_crimson_cuda_device.cmd"
    ...
    Write-Host "  runtime check script: $publishedRuntimeCheckScript"
    Write-Host "  runtime check wrapper: $publishedRuntimeCheckWrapper"
    Write-Host "  cuda device script: $publishedCudaDeviceScript"
    Write-Host "  cuda device wrapper: $publishedCudaDeviceWrapper"
    ...
    if ($PublishCurrent) {
        ...
        $currentRuntimeCheckScript = Require-Path -PathValue (Join-Path $currentRoot "check_crimson_runtime.ps1") -Label "Published current check_crimson_runtime.ps1"
        $currentRuntimeCheckWrapper = Require-Path -PathValue (Join-Path $currentRoot "check_crimson_runtime.cmd") -Label "Published current check_crimson_runtime.cmd"
        $currentCudaDeviceScript = Require-Path -PathValue (Join-Path $currentRoot "set_crimson_cuda_device.ps1") -Label "Published current set_crimson_cuda_device.ps1"
        $currentCudaDeviceWrapper = Require-Path -PathValue (Join-Path $currentRoot "set_crimson_cuda_device.cmd") -Label "Published current set_crimson_cuda_device.cmd"
        ...
        Write-Host "  runtime check script: $currentRuntimeCheckScript"
        Write-Host "  runtime check wrapper: $currentRuntimeCheckWrapper"
        Write-Host "  cuda device script: $currentCudaDeviceScript"
        Write-Host "  cuda device wrapper: $currentCudaDeviceWrapper"
    ...
    $publishedRuntimeCheckScript = Require-Path -PathValue (Join-Path $targetRoot "check_crimson_runtime.ps1") -Label "Published check_crimson_runtime.ps1"
    $publishedRuntimeCheckWrapper = Require-Path -PathValue (Join-Path $targetRoot "check_crimson_runtime.cmd") -Label "Published check_crimson_runtime.cmd"
    $publishedCudaDeviceScript = Require-Path -PathValue (Join-Path $targetRoot "set_crimson_cuda_device.ps1") -Label "Published set_crimson_cuda_device.ps1"
    $publishedCudaDeviceWrapper = Require-Path -PathValue (Join-Path $targetRoot "set_crimson_cuda_device.cmd") -Label "Published set_crimson_cuda_device.cmd"
