...
int main(int argc, char **argv) {
  ...
  const double archive_open_ms =
      std::chrono::duration<double, std::milli>(
          std::chrono::steady_clock::now() - archive_open_start)
          .count();
  if (!archive) {
    std::cerr << error << '\n';
    return 1;
  ...
  const double repository_open_ms =
      std::chrono::duration<double, std::milli>(
          std::chrono::steady_clock::now() - repository_open_start)
          .count();
  if (!repository) {
    std::cerr << error << '\n';
    return 1;
  ...
  const char *storage =
      ...
      : descriptor.storage == crimson::zarr::SubjectMaskStorage::Bitpacked
          ...
          : "rle";
  std::cout
      << "group=" << descriptor.source_group << " run=" << descriptor.run_name
      << " storage=" << storage << " crop_run=" << descriptor.source_crop_run
      << " rows=" << descriptor.row_count
      << " camera_frames=" << descriptor.camera_frame_count
      << " components=" << descriptor.component_labels.size()
      << " mask=" << descriptor.mask_width << 'x' << descriptor.mask_height
      ...
      << " crop_detection_indices_ms=" << metrics.crop_detection_indices_ms
      ...
      << " frame_index_initialize_ms=" << metrics.frame_index_initialize_ms
      << " frame_index_rows=" << metrics.frame_index_rows_read
      << " frame_index_source_bytes=" << metrics.frame_index_source_bytes
      << " frame_index_retained_bytes=" << metrics.frame_index_retained_bytes
      << " fallback_frame_index_builds=" << metrics.fallback_frame_index_builds
      << " fallback_frame_index_rows=" << metrics.fallback_frame_index_rows
      ...
      << " mapping_page_source_bytes=" << metrics.mapping_page_source_bytes
      << " cached_mapping_bytes=" << metrics.cached_mapping_bytes
      << " peak_mapping_bytes=" << metrics.peak_cached_mapping_bytes
      << " mapping_initialize_failures=" << metrics.mapping_initialize_failures
      << " max_mapping_page_ms=" << metrics.maximum_mapping_page_read_ms
      << " frame=" << frame << " status=" << static_cast<int>(resolution.status)
      << " detections=" << resolution.detections.size()
      << " present_components=" << present_components
      << " foreground_pixels=" << foreground_pixels
      << " contour_points=" << contour_points << " resolve_ms=" << elapsed_ms
