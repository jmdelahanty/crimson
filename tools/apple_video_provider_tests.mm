...
std::string makeFixturePath() {
  NSString *name =
      [NSString stringWithFormat:@"crimson-apple-video-provider-%@.mov",
...
std::filesystem::path makeFixtureDirectory() {
  NSString *name = [NSString
      stringWithFormat:@"crimson-apple-clip-index-%@", NSUUID.UUID.UUIDString];
...
bool writeFixture(const std::string &path, std::string *error) {
  ...
  if (writer == nil || ![writer canAddInput:input]) {
    *error =
        "construct: " + std::string(writer_error.localizedDescription.UTF8String
  ...
  if (![writer startWriting]) {
    *error =
        "start: " + std::string(writer.error.localizedDescription.UTF8String
  ...
  for (int frame = 0; frame < frame_count; ++frame) {
    ...
    const bool appended =
        [adaptor appendPixelBuffer:pixel_buffer
              withPresentationTime:CMTimeMake(frame, time_scale)];
  ...
  if (writer.status != AVAssetWriterStatusCompleted) {
    *error =
        "finish: " + std::string(writer.error.localizedDescription.UTF8String
...
bool writeFixtureWithRetry(const std::string &path, std::string *error) {
  for (int attempt = 0; attempt < 3; ++attempt) {
    [[NSFileManager defaultManager]
        removeItemAtPath:[NSString stringWithUTF8String:path.c_str()]
                   error:nil];
    if (writeFixture(path, error)) {
      return true;
    }
...
void testClipIndexedPlaybackBuffer() {
  ...
  const auto first = root / "clips/clip_000000/camera.mov";
  const auto second = root / "clips/clip_000001/camera.mov";
  std::filesystem::create_directories(first.parent_path());
  std::filesystem::create_directories(second.parent_path());
  std::string error;
  CHECK(writeFixtureWithRetry(first.string(), &error));
  CHECK(writeFixtureWithRetry(second.string(), &error));
  const auto clip = [](size_t index, int64_t start) {
    const std::string id = index == 0 ? "clip_000000" : "clip_000001";
    return json{{"clip_index", index},
                {"clip_id", id},
                {"recording_id", "apple_clip_fixture"},
                ...
                {"end_frame_exclusive", start + 12},
                {"first_clip_local_frame_index", 0},
                {"last_clip_local_frame_index", 11},
                {"frame_count", 12}};
  };
  const json document = {
      {"status", "ok"},
      ...
      {"camera_serial", "2010095"},
      {"clip_count", 2},
      {"source", {{"total_frames", 24}, {"fps", 10.0}}},
      {"clips", {clip(0, 0), clip(1, 12)}},
      {"checks",
       {{{"code", "metadata_rows_match_keyframe_total_frames"},
        ...
        {{"code", "clip_start_is_keyframe"}, {"status", "ok"}},
        {{"code", "clip_recording_frame_id_continuity"}, {"status", "ok"}}}}};
  const auto index_path = root / "recording_clip_index.json";
  {
  ...
  AppleVideoPlaybackBuffer playback;
  CHECK(playback.openClipIndex(index_path.string(), "clip-fixture", 4, &error));
  CHECK(playback.info().frame_count == 24);
  CHECK(std::fabs(playback.info().nominal_frame_rate - 10.0) < 0.01);
  CHECK(playback.waitForFrame(0, std::chrono::seconds(5)));

  CHECK(playback.requestSeek(10, &error));
  CHECK(playback.waitForFrame(10, std::chrono::seconds(5)));
  playback.setPlaybackState(10, true, 10.0);
  CHECK(playback.waitForFrame(12, std::chrono::seconds(5)));
  auto crossed = playback.frameForTarget(12, true);
  CHECK(crossed.has_value());
  CHECK(crossed->metadata.frame_number == 12);
  CHECK(crossed->metadata.local_frame_number == 0);
  CHECK(crossed->metadata.time_base.isValid());
  const double crossed_time_seconds =
      static_cast<double>(crossed->metadata.frame_pts) *
      static_cast<double>(crossed->metadata.time_base.numerator) /
      static_cast<double>(crossed->metadata.time_base.denominator);
  CHECK(std::fabs(crossed_time_seconds - 1.2) < 1e-6);

  playback.setPlaybackState(12, false, 10.0);
  CHECK(playback.requestSeek(17, &error));
  CHECK(playback.waitForFrame(17, std::chrono::seconds(5)));
  auto sought = playback.frameForTarget(17, true);
  CHECK(sought.has_value());
  CHECK(sought->metadata.frame_number == 17);
  CHECK(sought->metadata.local_frame_number == 5);
  CHECK(playback.metrics().active_clip_index == 1);
  CHECK(playback.metrics().clip_switches >= 1);
  playback.close();
...
void testRepresentativeAsset(const std::string &path,
  ...
  for (const GoldenFrame &expected : golden) {
    ...
    std::cout << "[AppleVideoRepresentative] target=" << expected.frame_number
...
int main(int argc, char **argv) {
  @autoreleasepool {
    if (argc == 3 && std::string(argv[1]) == "--write-fixture") {
      ...
      [[NSFileManager defaultManager]
          removeItemAtPath:[NSString stringWithUTF8String:output_path.c_str()]
                     error:nil];
      std::string fixture_error;
      if (!writeFixtureWithRetry(output_path, &fixture_error)) {
        std::cerr << "fixture creation failed: " << fixture_error << std::endl;
        return 1;
      }
    ...
    try {
      ...
      if ((argc == 3 || argc == 5) && std::string(argv[1]) == "--asset") {
        ...
        if (argc == 5 && std::string(argv[3]) == "--golden") {
          ...
          throw TestFailure{"usage: apple_video_provider_tests [--asset VIDEO "
                            "[--golden CSV]]"};
        }
