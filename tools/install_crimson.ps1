...
if ($SourceRoot -ieq $InstallRoot) {
    throw "SourceRoot and InstallRoot are the same: $SourceRoot"
}

$sourcePrefix = $SourceRoot + [System.IO.Path]::DirectorySeparatorChar
if ($InstallRoot.StartsWith($sourcePrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "InstallRoot must not be inside SourceRoot.`nSourceRoot:  $SourceRoot`nInstallRoot: $InstallRoot"
}
